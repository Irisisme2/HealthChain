...
function RecordView() {
  ...
  const [storedRecord, setStoredRecord] = useState('');
  ...
  const handleStoreRecord = async () => {
    try {
      ...
      // Pobranie zapisanego rekordu i ustawienie w stanie
      const fetchedRecord = await contract.methods.getRecord(account).call();
      setStoredRecord(fetchedRecord);
